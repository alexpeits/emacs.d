...
(defun my-blog-sitemap-format-entry (file-name style project)
  (let* ((project-plist (cdr project))
         (dir (file-name-as-directory
               (plist-get project-plist :base-directory)))
         (pubdir (file-name-as-directory
               (plist-get project-plist :publishing-directory)))
         (file-abs (concat dir file-name))
         (target-file-abs (concat pubdir file-name))
         (sitemap-sans-extension
          (plist-get project-plist :sitemap-sans-extension))
         (link (file-relative-name file-name))
         (link (if sitemap-sans-extension
                   (file-name-sans-extension link)
                 link))
         (title (org-publish-find-title file-abs project-plist))
         (date (format-time-string
                "%Y-%m-%d"
                (org-publish-find-date file-abs project-plist)))
         (preview (my-blog-get-preview file-abs))
         (regexp "\\(.*\\)\\[\\([^][]+\\)\\]\\(.*\\)")
         (rss-permalink (concat (file-name-sans-extension link) ".html"))
         (rss-pubdate (format-time-string
                       (car org-time-stamp-formats)
                       (org-publish-find-date file-abs project-plist))))
    (message "*****")
